...
window = ctk.CTk()
window.title("Gaussian Elimination Method")
...
def solve_equations():
    ...
    if success:
        ...
        for i in range(n):
            solution_labels[i].configure(text="  ".join(str(val) for val in result_matrix[i]))

        result = gaussian_elimination(result_matrix)
        if result:
