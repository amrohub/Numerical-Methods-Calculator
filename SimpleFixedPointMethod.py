...
def simpleFixed_Method():
    xNode = get_XNode()
    epsilon = get_Epsilon()

    x_IterationPlusOne = 0.0
    x_Iteration = 0.0
    error=0.0

    for i in range(100):    
        x_Iteration = xNode 
        x_IterationPlusOne = get_Function(xNode) 
        x_IterationPlusOneFloat = float(x_IterationPlusOne) 
        ...
        if i == 0 :
            table.insert(parent='', 
                        ...
                        values = (i, 
                                  round(x_Iteration,3), 
                                  round(x_IterationPlusOneFloat,3), " ")) 
        else:
            table.insert(parent='', 
                        index='end', 
                        iid=i, 
        ...
        if error > epsilon or i==0:
            error = abs((x_IterationPlusOneFloat - x_Iteration) / x_IterationPlusOneFloat) * 100 
            xNode = x_IterationPlusOneFloat 
